...
if (!empty($search)) {
    $sql = "SELECT * FROM register WHERE id = '$search' OR  username LIKE '%$search%'OR  email LIKE '%$search%'OR  full_name LIKE '%$search%'OR  phone LIKE '%$search%'   LIMIT $offset, $items_per_page";
} else {
    $sql = "SELECT * FROM register ORDER BY id DESC LIMIT $offset, $items_per_page";
}
...
if ($count == 0 && $current_page > 1) {
    ...
    header("Location: datasystem-manage-member.php?page=$previous_page&search_input=$search");
    exit();
}

// Construct the total items query
if (!empty($search)) {
    $total_items_query = "SELECT COUNT(*) as total FROM register WHERE id = '$search' OR  username LIKE '%$search%'OR  email LIKE '%$search%'OR  full_name LIKE '%$search%'OR  phone LIKE '%$search%' ";
} else {
    $total_items_query = "SELECT COUNT(*) as total FROM register";
}
            ...
            <div class="container pt-3">
                <div class="card ">
                    <div class="card-header pb-3" style="background-color:#2C3E50">
                        <h2 class="text-center text-light  pt-2"><i class="fa-solid fa-list me-2"></i>MANAGE-MEMBER</h2>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                ...
                                                <a href="datasystem-edit-member.php?id=<?php echo $row->id ?>&page=<?php echo $current_page ?>&search=<?php echo $search ?>" class="text-primary">
                                                    <i class="fa-solid fa-pen-to-square "></i></a>
                                            </div>
                                        </td>
                                        <form action="multipledelete-manage-member.php" method="POST" id="deleteForm">
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "deletequerystring-manage-member.php?idemp=" + id;
            }
        });
