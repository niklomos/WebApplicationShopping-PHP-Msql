...
if (!empty($search)) {
    $sql = "SELECT * FROM payment WHERE id = '$search' OR account_name LIKE '%$search%'  OR account_number LIKE '%$search%'   LIMIT $offset, $items_per_page";
} else {
    $sql = "SELECT * FROM payment ORDER BY id DESC LIMIT $offset, $items_per_page";
}
...
if ($count == 0 && $current_page > 1) {
    ...
    header("Location: datasystem-manage-payment.php?page=$previous_page&search_input=$search");
    exit();
}

// Construct the total items query
if (!empty($search)) {
    $total_items_query = "SELECT COUNT(*) as total FROM payment WHERE id = '$search' OR account_name LIKE '%$search%'  OR account_number LIKE '%$search%' ";
} else {
    $total_items_query = "SELECT COUNT(*) as total FROM payment";
}
            ...
            <div class="container pt-3">
                <div class="card ">
                    <div class="card-header pb-3" style="background-color:#2C3E50">
                        <h2 class="text-center text-light pt-2"><i class="fa-solid fa-list me-2"></i>MANAGE-PAYMENT</h2>
                            <div class="d-flex justify-content-between mb-2">
                                ...
                                <div class="d-flex justify-content-end ">
                                    <a href="datasystem-insert-payment.php" class="btn btn-success " style="border-width: 2px; "><i class="fa-solid fa-plus me-2 "></i>NEW </a>
                                </div>
                            </div>
                        ...
                        <table class="table table-striped table-hover ">
                            <thead class=" ">
                                <tr class="text-center ">
                                    ...
                                    <th class="text-start ">เลขบัญชี</th>
                            ...
                            <tbody class="text-center">
                                <?php while ($row = mysqli_fetch_object($result)) { ?>
                                    ...
                                    <tr>
                                        ...
                                        <td width="80px"><img src="images/<?php echo $row->images ?>" alt="" width="100%"></td>
                                        <td class="text-start "><?php echo $row->account_name ?></td>
                                        <td class="text-start "><?php echo $row->account_number ?></td>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                ...
                                                <a href="datasystem-edit-payment.php?id=<?php echo $row->id ?>&page=<?php echo $current_page ?>&search=<?php echo $search ?>" class="text-primary">
                                                    <i class="fa-solid fa-pen-to-square "></i></a>
                                            </div>
                                        </td>
                                        <form action="multipledelete-manage-payment.php" method="POST" id="deleteForm">
                                            <td width="5px">
                                                <div class="form-check">
                                                    <input type="checkbox" name="idcheckbox[]" id="" class="form-check-input" value="<?php echo $row->id; ?>">
                                                </div>
                                            </td>
                                    </tr>
                                <?php } ?>
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "deletequerystring-manage-payment.php?idemp=" + id;
            }
        });
