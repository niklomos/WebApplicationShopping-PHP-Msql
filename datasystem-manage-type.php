...
if (!empty($search)) {
    $sql = "SELECT * FROM type WHERE  type_name LIKE '%$search%'  LIMIT $offset, $items_per_page";
} else {
    $sql = "SELECT * FROM type ORDER BY id DESC LIMIT $offset, $items_per_page";
}
...
if ($count == 0 && $current_page > 1) {
    ...
    header("Location: datasystem-manage-type.php?page=$previous_page&search_input=$search");
    exit();
}

// Construct the total items query
if (!empty($search)) {
    $total_items_query = "SELECT COUNT(*) as total FROM type WHERE type_name LIKE '%$search%' ";
} else {
    $total_items_query = "SELECT COUNT(*) as total FROM type ";
}
            ...
            <div class="container  pt-3">
                <div class="card ">
                    <div class="card-header pb-3" style="background-color:#2C3E50">
                        <h2 class="text-center text-light pt-2 "><i class="fa-solid fa-list me-2"></i>MANAGE-TYPE</h2>
                        ...
                        <div class="d-flex justify-content-between mb-2 ">
                            ...
                            <div class=" justify-content-end ">
                                <a href="datasystem-insert-type.php" class="btn btn-success " style="border-width: 2px; "><i class="fa-solid fa-plus me-2 "></i>NEW </a>
                            </div>
                    ...
                    <div class=" card-body table-responsive p-0">
                        <?php if ($count > 0) { ?>
                            <table class="  table table-striped table-hover  0 ">
                                <thead class=" ">
                                    <tr class="text-start ">
                                        <th class="col-3">ลำดับ</th>
                                        <th class="col-6">ประเภทสินค้า</th>
                                ...
                                <tbody class="text-start ">
                                    <?php while ($row = mysqli_fetch_object($result)) { ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class=""><?php echo $row->type_name ?></td>
                                            <td>
                                                <div class="d-flex justify-content-center">
                                                    ...
                                                    <a href="datasystem-edit-type.php?id=<?php echo $row->id ?>&page=<?php echo $current_page ?>&search=<?php echo $search ?>" class="text-primary ">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                </div>
                                            </td>
                                            <form action="multipledelete-manage-type.php" method="POST" id="deleteForm">
                                                <td>
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "deletequerystring-manage-type.php?idemp=" + id;
            }
        });
