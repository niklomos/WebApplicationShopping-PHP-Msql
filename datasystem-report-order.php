...
if (isset($_POST['search'])) {
    ...
    $sql = "SELECT * FROM report_allorder WHERE id = '$search' OR product_name LIKE '%$search%'OR order_number LIKE '%$search%' OR full_name LIKE '%$search%'OR phone LIKE '%$search%'OR username LIKE '%$search%' LIMIT $offset, $items_per_page";
    ...
    $sql = "SELECT * FROM report_allorder ORDER BY id DESC  LIMIT $offset, $items_per_page";
    $result = mysqli_query($conn, $sql);
    $count = mysqli_num_rows($result);
    $order = $offset + 1; // Start order from the correct number
}

$total_items_query = "SELECT COUNT(*) as total FROM report_allorder";
            ...
            <div class="container pt-3">
                <div class="card ">
                    <div class="card-header pb-3" style="background-color:#2C3E50">
                        <h2 class="text-center text-light pt-2 pb-2"><i class="fa-solid fa-file me-2"></i>REPORT-ORDERS</h2>
                                    ...
                                    <?php while ($row = mysqli_fetch_object($result)) { ?>
                                        ...
                                        <tr class="text-start">
                                            ...
                                            <td class="text-center">
                                                <a href="datasystem-report-order-detail.php?id=<?php echo $row->id ?>  " class="text-primary ">
